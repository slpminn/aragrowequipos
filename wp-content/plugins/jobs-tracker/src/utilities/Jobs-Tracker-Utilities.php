...
/**
     ...
     * @return array|bool|null Associative array of statuses data if found, false if not found, or null on error.
     */
    public function getIndustries($id = 0) {
        
        if(JOBSTRACKER_DEBUG) error_log('JobsTrackerUtilities->getIndustries()');
        global $wpdb;
        
        // Define the table name with the WordPress prefix.
        $table_name = $wpdb->prefix . JOBSTRACKER_PREFIX . 'company_industries a';
        $select = "a.*";
        $from = "$table_name";
        /**
         * This query will order the results based on whether a record is a parent (industry_parent = 0) or a child, 
         * with parents coming first, followed by their respective children. Within each group, the records are 
         * further ordered by industry_parent, industry_name. 
         */
        $orderby = "ORDER BY 
            CASE
                WHEN industry_parent = 0 THEN ID  -- Parent records come first
                ELSE industry_parent              -- Children are ordered under their parents
            END,
            industry_parent, industry_name";
        // Check if $id is an array.
        if (is_array($id)) {
            // Prepare a placeholder for each ID in the array.
            $placeholders = rtrim(str_repeat('%d,', count($id)), ',');
            ...
            // Prepare SQL query to retrieve all.
            $sql = "SELECT $select FROM $from $orderby";
        }
    ...
    function getCompanies($id = 0) {
        ...
        $table_name3 = $wpdb->prefix . JOBSTRACKER_PREFIX . 'company_industries c';
        $select = "a.*, b.status_name, c.industry_name";
        $from = "$table_name
                    INNER JOIN $table_name2 ON a.company_status_id = b.ID
                    INNER JOIN $table_name3 ON a.company_industry_id = c.ID";
